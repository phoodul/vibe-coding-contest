...
| full (+expected_triggers) | (측정 진행 중) | | | | | |

**핵심 결론**: 4-way 비교에서 **alternating loop / similar RAG 모두 정답률 향상에 기여하지 않음**. RAG 는 오히려 정답률을 떨어뜨림.
...
- `docs/qa/kpi-evaluation-killer-full.json` (측정 진행 중 — 완료 시 본 보고서 갱신)
