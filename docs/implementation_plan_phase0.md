...
#### P0-13c: 시연 영상 1편 — 2024 수능 21번 라마누잔 풀이
- **What**: Legend Tutor 가 실제 수능 킬러 문제(2024 공통 21번)를 라마누잔 페르소나로 풀이하는 시연 영상 1편 제작. **사용자 직접 화면 녹화 + 음성 narration** (진정성 우선). 자료·스크립트·체크리스트는 운영자가 준비.
- **Why**: business-vision.md §3 차별화 무기 (Trigger 카드 시각적 노출). "정답 도달 핵심 코칭" 을 영상으로 보여주는 게 가장 빠른 신뢰 획득. 사용자 직접 발견(이번 세션) — 학부모 즉시 반응 예상.
- **How**:
  - 자료 준비 (운영자):
    1. 녹화 시나리오 — `docs/marketing/video-script-1.md` (라마누잔 introduction → 21번 문제 제시 → trigger 카드 노출 클로즈업 → 풀이 → R1 카드 마무리)
    2. 화면 녹화 가이드 — `docs/marketing/recording-guide.md` (OBS Studio 설정 / 해상도 1920×1080 / 음성 노이즈 가이드)
    3. 영상 제목·설명·해시태그 — SEO 최적 ("AI 가 수능 킬러 21번 푸는 방식 — 일타강사가 못 한 것")
    4. 자막 스크립트 (한국어 + 영어 옵션)
  - 사용자 액션:
    - 10분 long 영상 1편 (유튜브용)
    - 60초 short 1편 (인스타 릴스 / 틱톡용)
- **검증**:
  - 자료 3종 (script / guide / SEO 메타) 작성 완료
  - 사용자 녹화 1편 + short 1편 (Phase 0 종료 전)
- **선행 의존성**: P0-10 (1-pager — 영상 메시지 일관성)
- **예상 시간**: 자료 준비 4시간 / 녹화 4~6시간 (사용자)
...
- [ ] 텐볼스토리 콜드 메일 회신 ≥ 1 건 (또는 발송 완료)
- [ ] 영문법 PoC 5 문제 정답률 ≥ 70%
- [ ] 시연 영상 long 1편 + short 1편 업로드 완료 (유튜브·인스타)
