...
#### P0-13c: 시연 영상 1편 — 2024 수능 21번 폰 노이만 ↔ 학생 함께 풀이 + R1 리포트 (5분)
- **What**: Legend Tutor 가 실제 수능 킬러 문제(2024 공통 21번)를 **폰 노이만 페르소나가 학생과 함께 묻고 답하며 풀어가는 Socratic 대화** 와, **풀이 후 R1 리포트(정직성·trigger_motivation·학생 막힘 5 차원·풀이 정리)** 까지 모두 보여주는 시연 영상. 사용자 정정(2026-05-03): "함께 묻고 풀어가는 과정이 핵심 + 리포트도 반드시 보여줘야 한다 — 둘 다 필수".
- **Why**: business-vision.md §3 차별화 무기 두 축. **(1) 함께 풀이** = 콴다 답지 검색이 못 하는 1:1 코칭 가치. **(2) R1 리포트** = "답에 이르는 길의 명제화" — 일타강사가 구두로 흩뿌리는 직관을 LLM 명제로 정리. 이 두 축이 한 영상에서 모두 시각화돼야 학부모·학생이 "월 N만원 = 단과 학원 1과목 자리" 가치를 즉시 체감.
- **How**:
  - 영상 구성 (5분 long, 비율 = 함께 풀이 60% + 리포트 30% + 인트로/아웃트로 10%):
    1. 인트로 (~20초): 폰 노이만 인사 + 21번 문제 제시
    2. **함께 풀이 — Socratic dialogue (3분, ★ 핵심 가치)**: 폰 노이만이 학생에게 질문 → 학생 시도 → 폰 노이만이 trigger 카드로 다음 단계 유도 → 학생 답변 → ... 반복. 일방적 풀이가 아닌 양방향 대화. trigger 카드 노출 시점에 zoom-in. 학생 음성은 사용자 1인 2역(폰 노이만 + 학생) 또는 자막으로 처리.
    3. **R1 리포트 클로즈업 (1분 30초)**: 추론 트리 / trigger_motivation / 정직성 vs 학습 코치 / 학생 막힘 5 차원 / "왜 정답인지" 명제 — 한 카드씩 zoom-in narration
    4. 아웃트로 (~10초): "콴다는 답을, 우리는 함께 답에 이르는 길을 묻고 가르치고 정리한다"
  - 자료 준비 (운영자):
    ...
    3. SEO 메타 — 제목 "AI 와 함께 푼 수능 21번 + AI 가 정리한 리포트 — 일타강사가 못 한 것" / 해시태그 #수능 #AI튜터 #일타강사 #함께풀이 #리포트
...
- **검증**:
  ...
  - **함께 풀이 (Socratic dialogue) ≥ 50% 분량** + R1 리포트 ≥ 25% 분량
...
- [ ] 텐볼스토리 콜드 메일 회신 ≥ 1 건 (또는 발송 완료)
- [ ] 영문법 PoC 5 문제 정답률 ≥ 70%
- [ ] 시연 영상 long 5분 + short 40초 업로드 완료 (유튜브·인스타) — 함께 풀이 ≥ 50% + R1 리포트 ≥ 25% 분량
