...
#### P0-13c: 시연 영상 1편 — 2024 수능 21번 폰 노이만 풀이
- **What**: Legend Tutor 가 실제 수능 킬러 문제(2024 공통 21번)를 **폰 노이만 페르소나** (사용자 결정 2026-05-03) 로 풀이하는 시연 영상 1편 제작. **사용자 직접 화면 녹화 + 음성 narration** (진정성 우선). 자료·스크립트·체크리스트는 운영자가 준비.
- **Why**: business-vision.md §3 차별화 무기 (Trigger 카드 시각적 노출). "정답 도달 핵심 코칭" 을 영상으로 보여주는 게 가장 빠른 신뢰 획득. 폰 노이만은 게임이론·계산 집약 분야 강함 → 21번 같은 분석적 풀이에 어울리는 페르소나 톤.
- **How**:
  - 자료 준비 (운영자):
    1. 녹화 시나리오 — `docs/marketing/video-script-1.md` (폰 노이만 introduction → 21번 문제 제시 → trigger 카드 노출 클로즈업 → 풀이 → R1 카드 마무리)
...
| P0-13c | (pending) | | 시연 영상 1편 (폰 노이만 / 2024 21번) |
