...
| 리스크 | 대응 |
|---|---|
| 베타 1명 인터뷰가 늦어져 P0-01~04 가 placeholder 만으로 끝남 | 인터뷰 즉시 후속 commit 으로 추가 (Phase 0 종료 전 반드시) |
...
- **How**:
  ...
  - 1명 인터뷰가 아직이면 데이터 분석 결과 + "인터뷰 후 보완 필요" placeholder 명시
- **검증**:
...
#### P0-04: persona 응답 일관성 + 인터뷰 후 보완 placeholder
- **What**: 5거장 페르소나 (라마누잔·가우스·폰 노이만·오일러·라이프니츠) 의 응답 톤·길이가 일관되지 않은 케이스가 P0-01 에서 발견될 수 있음. system prompt 의 페르소나 톤 가이드를 보강한다. 동시에 베타 1명 인터뷰가 아직이면 **"인터뷰 후 task 채우기" placeholder commit** 을 명시한다.
...
- **How**:
  ...
  - placeholder: `docs/qa/beta1-interview-todo.md` — 인터뷰 후 추가 결함을 P0-04b, P0-04c 등으로 추가 commit 하라는 명시
- **검증**:
  - persona 5명 각각 동일 입력에 대해 응답 길이 ±20% 이내 / 어휘 톤 통일 (수동 sampling)
  - `docs/qa/beta1-interview-todo.md` 존재
- **선행 의존성**: P0-01
- **예상 시간**: 반나절
- **Commit 메시지 prefix**: `fix:` 또는 `chore:` (인터뷰 placeholder)
...
#### P0-15: 추가 베타 4명 모집 채널 확정 + 송출
- **What**: 베타 5명 cap 을 채우기 위한 4명 추가 모집을 시작한다. 채널 후보: ① 지인 4명 (수만휘 정회원 자격 보유 수험생) ② 수만휘 게시판 모집글 (운영자 동의 시) ③ 인스타·X 광고 (소액).
- **Why**: roadmap.md §Phase 0 산출물 4 (베타 5명 가입 완료 100%) + research_raw.md §6 자발적 바이럴.
- **How**:
  - 모집글 템플릿: `docs/marketing/beta-recruitment-post.md` 신규 (3 채널용 본문)
  - `/legend/beta/apply` 페이지 점검 — 13차 Δ27 후속에서 가시성·N수생 옵션 fix 완료 (커밋 1dffc3a)
  - 모집 대시보드: admin 에서 `pending` 신청 실시간 모니터링
- **검증**:
  - 4명 추가 신청 접수 (Phase 0 종료 시점)
  - 5명 모두 가입·온보딩 완료 → 1턴 이상 사용
- **선행 의존성**: P0-14, P0-13 (자발 후기 가이드 사전 준비)
- **예상 시간**: 반나절 (모집글 + 송출)
- **Commit 메시지 prefix**: `docs:` (모집글) + `chore:` (모집 진행)
...
- **How**:
  ...
  - 구성:
    ...
    - **Day 14**: 누적 풀이 ≥ 20문제 + 만족도 1차 인터뷰 (15분)
    - **Day 30**: 최종 인터뷰 + 자발 후기 게시 안내 (강요 X)
  - 운영자 액션: 매 단계 누락 시 카카오톡·이메일 1:1 안내
- **검증**:
  - 5명 모두 Day 1 통과 → 1턴 이상 사용
  - Day 14 인터뷰 기록 ≥ 4 / 5 (80%)
- **선행 의존성**: P0-15
- **예상 시간**: 2시간
- **Commit 메시지 prefix**: `docs:`
...
- **Day 4~7**: B 전체 + C 의 P0-10·P0-11·P0-13 병렬
- **Day 8~10**: C 의 P0-12 + D 전체 (P0-14 → P0-15 → P0-16)
- **Day 11~14**: 회귀 + 회고 + 누락 task 보완
...
| Week 1 | 5/6 (화) | P0-04 (persona + 인터뷰 placeholder) | A 종료 |
...
| Week 2 | 5/12 (월) | P0-11 (SEO) + P0-13 (수만휘 가이드) 병렬 | |
| Week 2 | 5/13 (화) | P0-12 (텐볼스토리 콜드 메일) + P0-14 (1건 승인) | C 종료 |
| Week 2 | 5/14 (수) | P0-15 (4명 모집 송출) | |
| Week 2 | 5/15 (목) | P0-16 (온보딩 체크리스트) | D 종료 |
...
- [ ] **C 카테고리**: 1-pager + 텐볼스토리 콜드 메일 + 수만휘 가이드 + SEO 강화 4개 산출물 완료
