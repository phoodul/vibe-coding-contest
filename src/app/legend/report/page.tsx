...
export default function LegendReportPage() {
  ...
  if (!authorized) {
    return (
      <main className="mx-auto max-w-3xl px-4 py-16 text-center text-white">
        ...
        <Link href="/login?next=/legend/report" className="text-violet-400 underline mt-4 inline-block">
          로그인하러 가기
        </Link>
      </main>
    );
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-indigo-950 to-violet-950 text-white">
      <div className="max-w-5xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold">이번 주 레전드 리포트</h1>
            <p className="text-sm text-white/60 mt-1">
              최근 7일간의 풀이를 분석한 약점 진단입니다. 매주 갱신돼요.
            </p>
          </div>
          <Link href="/legend" className="text-sm text-white/60 hover:text-white">
            ← 채팅
          </Link>
        </div>
