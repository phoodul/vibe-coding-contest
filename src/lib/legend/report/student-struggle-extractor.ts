...
3. **trigger_quote** (6~10 문장) — Tool 과 Trigger 의 엄격한 분리. Trigger 의 본질은 "왜 하필 이 도구인가의 필연성·개연성"
   ...
   **Tool (수학적 도구)** = 학생이 떠올렸어야 한 **구체적 세부 수단** (WHAT).
   - "치환·보조선·여사건·부분적분" 같은 단원 이름 X. 더 세밀하게.
   - 진짜 tool 예: "\\(u = x-1\\) 로 두는 치환", "원의 중심에서 현에 내린 수선의 발", "\\(g'(x)=0\\) 의 해를 통해 극값 후보 잡기".

   **Trigger** = 무수한 가능 도구 중 **왜 하필 이 도구를 떠올려야 했는가의 필연성·개연성** (WHY).
   - 학생이 막혔던 자리에서 머릿속에 그 특정 도구가 **자연스럽게 호출되어야 했던 인과적 근거**.
   - 단순 도구 이름 X. **그 도구가 떠올라야 했던 이유의 인과 사슬**.

   ### Trigger 를 구성하는 4 요소

   **(1) Tool 식별** — 학생이 떠올렸어야 한 정확한 도구 (세밀하게).

   **(2) 신호 (signal)** — 문제 표면의 어떤 단서가 그 도구를 부르는가? 학생이 봤어야 했지만 못 본 표현·기호·조건.

   **(3) 개연성 / 필연성 (necessity)** — Trigger 의 핵심.
     - **개연성**: 그 신호가 보일 때 가능한 여러 도구 중 이 도구가 가장 효율적·자연스러운 이유.
     - **필연성**: 그 신호의 수학 구조가 이 도구만의 적용을 보장·강제하는 이유.
     - 다른 가능 도구와의 비교 필수: "직접 전개·미분·대입 같은 대안도 시도 가능했지만, 그것들이 비효율적이거나 작동하지 않는 이유".

   **(4) 인식 구조 (recognition pattern)** — 학생이 다음에 같은 신호를 보면 자동으로 같은 도구가 떠오르도록 신호-도구 연결을 어떻게 내재화하는가.

   ### 단 하나의 절차 적용 시연
   합성함수 \\(f(x-1)\\) 반복 등장 자리에서 학생이 막혔던 경우:
   "**Tool**: 학생은 \\(u = x-1\\) 로 두고 \\(f(u)\\) 로 재기술하는 치환을 떠올렸어야 합니다. **신호**: 함수 인자가 단순 변수가 아닌 \\(x-1\\) 같은 일차식으로 반복되는 것이 표면 단서였습니다. **개연성·필연성**: 같은 인자가 반복되는 구조에서는 인자를 통째로 새 변수로 보는 것이 사고를 가장 단순화시킵니다. 직접 전개나 미분으로 풀 수도 있었지만, 표현식이 복잡할수록 비효율이 누적되고 결국 치환을 거치게 되므로, 처음부터 치환이 자연스럽고 거의 불가피한 선택이 됩니다. **인식 구조**: '합성함수 인자가 일차식·익숙한 형태로 반복' 신호 → '인자를 새 변수로 두는 치환' 도구. 이 신호-도구 연결을 학생이 자동 호출되도록 새기면 다음에 같은 신호 만났을 때 머릿속에서 치환 사고가 즉시 일어납니다."

   ### 절대 금지
   - 추상 메타: "양방향 trigger", "역행 trigger", "forward/backward 모두 사용", "trigger 발동 패턴"
   - 단원 이름만: "치환을 떠올렸어야 한다" — 어떤 치환인지 세밀히 명시 안 하면 의미 없음.
   - Trigger 와 Tool 혼동: "이 trigger 는 치환이다" 는 틀림. Trigger 는 "치환이 떠올라야 했던 이유의 인과 사슬" 이지 치환 자체가 아님.
