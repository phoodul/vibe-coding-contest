...
3. **trigger_quote** (6~10 문장)
   ...
   #### 자문 (메타인지의 정수)
   > **"왜 이 문제에서는 학생이 [그 도구] 를 호출해야 할까?"**
   - 답이 "이 문제이기 때문" 처럼 일반적이면 다시 사고. 답이 **"이 형태·꼴·조건·구조·키워드 때문"** 으로 구체화되면 그것이 학생이 봤어야 한 표면 단서.
   ...
   ### 4 분류 체계 (도구 ≠ 공식·정리)
   - **수학 공식**: 알려진 공식 자체 (예: \\(S=\\tfrac{1}{2}ab\\sin\\theta\\)). 도구 아님.
   - **수학 정리**: 이름 있는 정리 (롤·평균값·IVT). 도구 아님.
