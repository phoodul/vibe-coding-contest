...
## 출력 (반드시 JSON 만, 다른 텍스트·코드펜스·해설 금지)
{
  ...
  "trigger_motivation": "...",
  "structured_trigger": {
    "cue_a": "이 문제의 정확한 cue (40~80자) — 예: '∞/∞ 꼴 유리함수 극한', '두 변과 끼인각이 주어진 삼각형'",
    "tool_b": "호출되어야 할 구체 도구 (40~80자) — 예: '분모 최고차항으로 분모·분자 나누기', 'S=½ab sinθ 적용'",
    "why_text": "왜 그 cue 가 그 tool 을 부르는가의 인과 (200자 내외)"
  }
}

각 필드는 비워두지 마세요. 풀이 정보가 부족해도 문제 유형으로부터 추론해서라도 채우세요. 학생이 "정리하지 못했습니다" 메시지를 보지 않도록.

structured_trigger 는 trigger_motivation 자연어 문단과 동일한 trigger 의 구조화 형식입니다. cue_a 는 학생이 노트에 적을 정석 좌변, tool_b 는 우변. 풀이 종료 후 trigger 라이브러리에 자동 누적되므로 정확하고 일반화 가능한 표현으로 작성하세요.`;
...
function parseSummaryJson(raw: string): SolutionSummary {
  ...
  try {
    ...
    // Δ23 — structured_trigger 검증 (3 필드 모두 string 이고 비어있지 않으면 보존)
    const st = (obj as { structured_trigger?: unknown }).structured_trigger;
    let structured: SolutionSummary['structured_trigger'];
    if (st && typeof st === 'object') {
      const s = st as { cue_a?: unknown; tool_b?: unknown; why_text?: unknown };
      if (
        typeof s.cue_a === 'string' &&
        s.cue_a.trim() &&
        typeof s.tool_b === 'string' &&
        s.tool_b.trim() &&
        typeof s.why_text === 'string' &&
        s.why_text.trim()
      ) {
        structured = {
          cue_a: s.cue_a.trim().slice(0, 200),
          tool_b: s.tool_b.trim().slice(0, 200),
          why_text: s.why_text.trim().slice(0, 400),
        };
      }
    }
    return {
      ...
      ...(structured ? { structured_trigger: structured } : {}),
    };
  } catch {
