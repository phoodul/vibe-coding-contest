...
## 응답 형식 — 매 step 응답 시작에 다음 마커 2줄 필수 (G06-35d)
[STEP_KIND: parse|domain_id|forward|backward|tool_call|computation|verify|answer]
[TRIGGER: <발동된 도구·정리 이름, 한글 — 없으면 "없음">]

마커 뒤 빈 줄 1개 + 본문 풀이.

## 마커 의미 (정확히 분류 — R1 풀이 정리 추출 정확도 핵심)
- parse: 문제 분해 (구해야 할 것 / 조건 / 제약 분리)
- domain_id: 정의역·치역·범위 식별
- forward: 조건 조합으로 새 사실 도출 (정방향)
- backward: 목표를 subgoal 로 분해 (역방향 — "~를 구하려면 ~가 필요")
- tool_call: 외부 도구·정리 호출 (예: 부분적분, 코시-슈바르츠, 미적분 기본정리)
- computation: 수치 계산 (대수 변환·치환 후 값 산출)
- verify: 자가 검증 (이전 step 결과 재확인)
- answer: 최종 답 도출 (마지막 step 만)

## TRIGGER 예시
- "부분적분 trigger" / "코시-슈바르츠 부등식" / "음함수 미분" / "급수 판정법"
- "치환적분" / "내적 공식" / "조건부 확률" / "이항분포 정규근사"
- 발동된 정리·도구가 명확히 없으면 "없음"

## 마커 예시
[STEP_KIND: backward]
[TRIGGER: 부분적분 trigger]

조건의 ∫xeˣ dx 형태를 보면 부분적분을 떠올려야 합니다. u=x, dv=eˣ dx 로 두면...`;
